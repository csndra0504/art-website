...
export function EmailSignupBanner() {
	...
	const handleSubmit = async (e: FormEvent) => {
		...
		try {
			...
			trackLead('email_signup', { method: 'banner' });
			setStatus('success');
		} catch {
			setStatus('error');
		}
		...
		>
			<Container size="lg" py={10}>
				<Group justify="space-between" wrap="nowrap" gap="sm" align="center">
						...
						<form onSubmit={handleSubmit} noValidate style={{ flex: 1, minWidth: 0 }}>
							<Group gap="sm" wrap="wrap" align="center">
								<Text size="sm" style={{ lineHeight: 1.5, flex: '1 1 220px' }}>
									Join my email list for new originals &amp; print releases - plus where to
									find me around Pittsburgh.
								</Text>
								<Group gap="xs" wrap="nowrap" style={{ flex: '1 1 260px', minWidth: 240 }}>
									<TextInput
										type="email"
					...
					<CloseButton size="sm" aria-label="Dismiss signup banner" onClick={handleDismiss} />
				</Group>
			</Container>
		</div>
	);
